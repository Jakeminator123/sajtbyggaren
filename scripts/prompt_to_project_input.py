...
# ---------------------------------------------------------------------------
# Hero-headline carry-forward (B173)
# ---------------------------------------------------------------------------
...
# heroHeadline > blueprint > company.name, fb9692d) — but the field is only
...
# itself flip appliedVisibleEffect; an explicit "byt rubriken till X" still
# wins because _apply_copy_directives mirrors the new tagline over the pin.
# Init generation is untouched (the pin only runs in generate_followup).


def _latest_run_dir_for_site(runs_dir: Path, site_id: str) -> Path | None:
...
def _run_blueprint_hero_headline(run_dir: Path) -> str | None:
...
_HERO_HEADLINE_PIN_MAX_LENGTH = 200


def _pin_previous_hero_headline(
    ...
    *,
    ...
    if isinstance(existing, str) and existing.strip():
        return
    headline = _run_blueprint_hero_headline(run_dir)
    if headline is None:
    ...
    if headline is None or len(headline) > _HERO_HEADLINE_PIN_MAX_LENGTH:
        return
