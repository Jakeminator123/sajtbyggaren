...
`main` är vid `ed1d743` (`docs(scout): finalize handoff after auto-merge-pipeline + B136 fix`) efter en auto-merge-pipeline 2026-05-19 som stängde B130/B131/B132/B133/B134/B135/B136 + fixade F2 stale-SHA-referensen och drev en delvis avslutad Viewser-overlay-E2E-Scout. Snitt 7.1/10 över tre case (Case 1-3a) — över beslutsregelns 7-tröskel. Discovery-kedjan från B121 (merge `e3fa67b` PR #34–#37) är fortfarande sealed. **Direkt nästa steg:** något av (a) fortsätt Scout med Case 4/6/3b/Spår B, (b) variant-promotion-sprint för B129 (8 nya scaffold-variants är dead-on-arrival via `_DEFAULT_VARIANT_BY_SCAFFOLD`-guarden), (c) B125 browser-fallback-ADR (produktblockare innan extern kund), eller (d) Page Intent Variant B (extra-routes från `_PAGE_TO_ROUTE_HINT`) — operatören väljer.
