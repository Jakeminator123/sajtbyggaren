...
Last verified state: `8fae26a` (2026-05-14, B56/B57/B58 landade i samma commit: B56 säkrar StackBlitz-preview med `next dev --webpack` via in-memory package.json-patch i `apps/viewser/lib/stackblitz-files.ts`, B57 härdar `.env*`-tracking-guarden i `test_viewser_env_file_is_not_committed`, B58 allowlistar `.env.example` i StackBlitz-uploadfiltret. Inga öppna PRs.)
