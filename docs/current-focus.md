...
**Steward Pass 2 (2026-05-19, sen kväll, docs-only ovanpå `1d6fadf`):** Efter Scout RO-rapport PR #47 (`docs/reports/scout-wizard-tagline-pagecount-tone-2026-05-19.md`, mergad i `1d6fadf`) uppdaterades fix-pekarna för B137 och B139, och två nya öppna ytterligare fynd registrerades: **B137 fix-pekare** flyttad från `scripts/prompt_to_project_input.py:_derive_tagline` till `packages/generation/discovery/resolve.py:_apply_company_fields` (rad 609-628) — wizardens `answers.offer` skriver över briefens tagline efter att briefen producerats; befintlig `_derive_tagline` kvarstår som fri-prompt-fallback. **B139 fix-pekare** flyttad från `packages/generation/codegen/` till `scripts/build_site.py:variant_css()` (rad 701-737) + `patch_globals_css()` (rad 2107-2136); helpern läser bara `variant["tokens"]`. **B140 (Låg, ny):** `brand.primaryColorHex` ignoreras av `variant_css` — angränsande till B139 (samma helper) men separat data-kanal (explicit operatörshex vs extraherad tone). **B141 (Låg-medel, ny):** `packages/generation/planning/plan.py:_assemble_generation_package()` skriver bara `siteBriefRef`, INTE `siteBrief`-objektet, så `packages/generation/codegen/codegen.py:_summarise_generation_package()` läser `tone`/`businessType` från död pipeline. Aktuell buggräkning: **30 aktiva, 0 misplaced, 5 unknown, 98 stängda** (B140 + B141 nya). NOTE-blockquoten i B137-entryn från Pass 1 är borttagen (rätt kodväg nu inlagd). Inga öppna PRs, inga lokala feature-branches. Inga produktfiler rörda — Builders sprint öppnar nästa pass. SHA-bumpen av Last verified-raden nedan sker i uppföljnings-commit (Standard loop steg 8).
