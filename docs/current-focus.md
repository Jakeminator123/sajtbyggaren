...
Last verified state: `e1acab9` (2026-05-14, docs reconciliation efter B56/B57/B58. B56/B57/B58 landade i `8fae26a` på `main`: B56 säkrar StackBlitz-preview med `next dev --webpack` via in-memory package.json-patch i `apps/viewser/lib/stackblitz-files.ts`, B57 härdar `.env*`-tracking-guarden i `test_viewser_env_file_is_not_committed`, B58 allowlistar `.env.example` i StackBlitz-uploadfiltret. `e1acab9` är docs-only-reconciliation som flyttar B56 från Öppna till Stängda i `known-issues.md` och rensar inaktuella stash-omnämnanden i `current-focus.md` + `handoff.md`. Stale B56-stash droppad, no-op-branch `fix/b56-stackblitz-webpack-dev` raderad lokalt + remote. Inga öppna PRs.)
