...
Last verified state: `0f3bd67` (2026-05-26 PM UTC, post-C4 backend-handoff audit — cloud-grind PR #121 mergad lokalt med konfliktlösning).
...
3. **Backend-Gap fixar baserade på C4-audit** (cloud-grind levererade noggrann audit 2026-05-26 i PR #121, `0f3bd67`). Auditen bekräftade 5 stängda (1, 2, 3, 8, 11), 5 delvis (4, 5, 6, 7, 9) och 1 öppen (10). Konkreta nästa fixar i prioritetsordning:
   - **Gap 5 quick win** (~1h, S): persist `directives.notesForPlanner` (mappad i `apps/viewser/components/discovery-wizard/wizard-payload.ts:496-514`) till Site Brief / planner-input. Spår saknas i `scripts/prompt_to_project_input.py` + Discovery Resolver. Site Brief-fältet finns redan (`packages/generation/brief/extract.py:167-170,313`).
   - **Gap 4 quick win** (~1-2h, S): merga `directives.requestedCapabilities` (från `wizard-payload.ts:406-418`) deterministiskt i `_resolve_capabilities()` (`packages/generation/discovery/resolve.py:1325-1362`). Idag läses bara `answers.mustHave` + taxonomy + befintlig brief.
   - **Gap 6 + 7 paired sprint** (~3-4h, M): build-pipeline-konvertering. Gap 6 = multi-size `public/favicon.ico` från `media.favicon` (Next metadata `icons` redan rendrad i `packages/generation/build/renderers.py:313-331`). Gap 7 = center-crop till `public/og-image.png` 1200×630 från `media.ogImage` (Next OG-metadata redan i `renderers.py:336-367`). Båda kräver `sharp` eller `Pillow` i build.
