...
Ingen pågående produktimplementation på `main`. Backoffice-kontrollplanen är nu hel: PR #32-cherrypick-serien lyfte in kontrollplan/Doctor/impact-vy/selection-profiles/variant- och dossier-candidate-ytor, och `0fe353f` stängde två post-PR-#32-fynd från extern review (B126 dossier-graf-nyckel-mismatch + B127 Doctor-villkor inverterat). Tidigare landade starter dependency hardening i `1c68035` (stängde B108: ren `npm audit`-baseline + om-installation vid ändrade package-inputs), Backoffice control-plane v0 i `860e553`, och demo-baseline-fix 1E i `bc43eb8` (stängde B105/B106/B107). B101/B102 lämnas öppna med låg prioritet (commerce-CTA route/text-mismatch i `render_home`/`render_products`), liksom B97/B98/B89-B93 och bug-sweep-resterna. Nästa konkreta uppgift är **B121 — Discovery Resolver / canonical discovery source-of-truth** — se "Next action".
...
**B121 — Discovery Resolver / canonical discovery source-of-truth.**
Bygg en deterministisk "wizard wins, sedan brief, sedan LLM-default"-
resolver så att discovery-sanningen inte längre splittras i fyra lager
utan explicit konfliktlösning. Bakgrund: idag passerar fälten via
(1) wizardens `WizardAnswers` i
...
wizardens deterministiska fält — och "sista vinner"-ordningen gör att
samma input kan rendra olika beroende på vilket lager som ändrades
sist. Det öppnar för "varför blev fält X så här fast jag fyllde i
Y?"-buggar.

Konkreta steg:

1. Definiera resolver-kontraktet per fält (företagsnamn, kontakt,
   plats, services, brand-tokens, conversion goals, language): vilket
   lager vinner när fältet är ifyllt vs tomt, och vad räknas som
   "ifyllt" (icke-tom sträng / icke-tom lista / explicit `null`).
2. Implementera resolver-helpern som **delad** modul (sannolikt under
   `packages/generation/discovery/` eller motsvarande — bekräfta
   placeringen mot repo-boundaries; om den hör hemma i
   `packages/generation/` får B121-sprinten ta en mini-ADR ovanpå
   ADR 0017/0019 som beslutar exakt path, för att inte krocka med
   B13a arkitektur-flytt).
3. Trådar in resolvern på alla fyra ställen så samma fält-resolution
   körs oavsett om vägen är wizard-only, prompt-only eller mixed.
4. Regressionstester som låser ordningen för minst:
   företagsnamn (wizard-ifylld vs brief-extraherat vs LLM-default),
   plats (wizard `Sverige` vs brief `Malmö`), services (wizard tom
   lista vs brief 3 träffar), brand-tokens.
5. Efter B121 är klart: **backoffice discovery control / mapping
   review** — en vy som visar `wizardAnswers` ⇄ `brief` ⇄ `appliedOverrides`
   ⇄ `finalProjectInput` per run, så operatören kan inspektera vilken
   källa som vann per fält. Egen sprint, ej blocker för B121 självt.

**Beslutsregel innan implementation:** kontrollera att discovery-
sanningens placering inte krockar med B13a (arkitektur-flytt av
`scripts/build_site.py` till `packages/generation/build/`). Resolvern
hör hemma på samma logiska sida som den eventuella build-flytten;
landar B121 i `packages/` är det rimligt att samma sprint dokumenterar
en mini-ADR. Sätt inte resolvern i `scripts/`-shells — det blir
samma split-sanning-skuld igen.
...
1. **B121 — Discovery Resolver / canonical discovery source-of-truth**
   (Medel arkitekturskuld, registrerad post-PR-#31 runda 2). Bygg
   delad resolver-helper med tydlig "wizard wins, brief, LLM-default"-
   ordning per fält + regressionstester + bestäm placering mot
   repo-boundaries (mini-ADR om resolvern landar i
   `packages/generation/`). Se "Next action" för konkreta steg.
2. **Backoffice discovery control / mapping review** — uppföljare till
   B121. Vy som visar `wizardAnswers` ⇄ `brief` ⇄ `appliedOverrides` ⇄
   `finalProjectInput` per run så operatören kan inspektera vilken
   källa som vann per fält. Egen sprint, ej blocker för B121 självt.
