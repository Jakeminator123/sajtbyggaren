...
- **PR #38** `feat(variants): add eight scaffold variants (variantModel)`
  (`feat/eight-scaffold-variants`, commit `4cd1058`, +755/-1 över 18 filer).
  Skapad 2026-05-18. Innehåller åtta nya canonical Scaffold Variants
  (4× `local-service-business`, 4× `ecommerce-lite`) genererade via
  `variantModel`/gpt-5.4 + speglade i `data/variant-candidates/` för
  backoffice-review. Alla `enabled: true`. **Blockad per coach-direktiv
  2026-05-19**: ingen variant-promotion under Steward eller Scout —
  PR:n saknar variant-selection-logik så discovery taxonomy defaultar
  fortfarande till `nordic-trust`/`clean-store` och de nya variants
  kan inte väljas av planning/build i prod-flödet. Hör hemma i en
  egen variant-promotion-sprint (se Queue #6). Lämna öppen utan
  review-arbete tills sprinten beslutas. PR #25 `cursor/env-setup-9fef`
