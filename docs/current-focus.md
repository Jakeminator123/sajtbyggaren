...
Last verified state: `6516c28` (2026-05-13, B13 route-emission implementation + ro-review fixup, awaiting PR review)
...
(commits `6a0a1c5` + `6516c28`). `scripts/build_site.py:write_pages`
läser nu `scaffold_routes["defaultRoutes"]` och dispatchar per route id
i stället för att hårdkoda `/tjanster`. Smoke-testet
...
`feat/b13-route-emission` (lokal branch, ska pushas och göras till PR
för Bugbot-review). HEAD `6516c28`.

## Next action

**PR-flöde för B13:** push `feat/b13-route-emission` till origin,
öppna PR mot `main`, vänta in Bugbot-rapport, åtgärda eventuella
findings, merge (squash). Därefter Standard loop steg 7: bumpa
SHA:n här till mergekommiten på `main`.

## Blocked items

- **Aktivering av `ecommerce-lite -> commerce-base`** - fortfarande
  blockerad av B13 fram tills `feat/b13-route-emission` är mergad.
  `SCAFFOLD_TO_STARTER` i `packages/generation/planning/plan.py` står
  kvar med `ecommerce-lite: marketing-base` tills B13 ligger på `main`
  och en uppföljande PR flippar mappingen + uppdaterar
  `data/starters/README.md` och `docs/known-issues.md` B20-posten.

## Do not start yet

- PR #17 (`frontend/christopher-import`) - ligger draft, ska inte
  granskas eller mergeas förrän B13-PR är mergad.
...
1. Push `feat/b13-route-emission` + öppna PR + invänta Bugbot +
   åtgärda findings + merge.
2. Aktivera mapping `ecommerce-lite -> commerce-base` (separat PR:
   ...
   `atelje-bird` byggas mot `commerce-base` när codegenModel-scope
