...
Last verified state: `e67cd90` (2026-05-18, direktpushad `docs: write handoff for next Steward pass`). Detta är en handoff-close-bump: ingen ny produktcommit sedan `adde45c` (B112-fix). Steward-handoff-sektionen i `docs/handoff.md` nu uttryckligen adresserar nästa Steward-pass med (1) sammanfattning av detta agentpass fyra ron (Steward bug-scope-cleanup `7742d39`/`0d3e9b8`, B109 reviewer-hotfix `fa277a1`/`b3800ca`, B112 reviewer-triage `adde45c`/`9bf3893`, handoff-close `e67cd90`), (2) två öppna B-IDs med specifik scope-vägledning — B110 (normaliseringsglapp; kräver delad helper i samma sprint som B13a-flytt) och B111 (variantModel `--strict`-flagga; lågprio enhancement), (3) anti-instruktioner (inte halvfixa B110, inte dubbel-IDa befintliga fynd, inte acceptera misplaced > 0), (4) parkerat operatörsförslag om Steward-automation i tre nivåer (mini-bot pre-push hook, GitHub Action, Cloud Agent). Aktuell buggräkning: 19 aktiva, 0 misplaced, 6 unknown, 79 stängda. Föregående mainline-commits: `9bf3893` (Steward-bump efter B112-fynd), `adde45c` (B112-fix), `b3800ca` (Steward-bump efter B109), `fa277a1` (B109-fix), `0d3e9b8` (Steward-bump efter scope-cleanup), `7742d39` (Steward bug-scope cleanup), `1c68035` (B108-fix), `860e553` (`feat(backoffice): add asset control plane`). Guards gröna inför handoff-close: `python scripts/check_term_coverage.py --strict`, `python -m pytest tests/test_bug_scope_discipline.py tests/test_docs_freshness.py tests/test_no_legacy_terms.py -q`, `python scripts/list_open_bugs.py --quiet`. `backup-26-VIKTIG` (pre-B108), `backup-27` (post-B108, pre-cleanup), `backup-28` (post-cleanup, pre-B109) och `backup-29` (post-B109, pre-B112) finns pushade på origin; inget nytt backup-N skapades för detta handoff-pass eftersom det är ren docs-only. Alla lokala branches utom `main` är raderade på operatörens uttryckliga begäran.
