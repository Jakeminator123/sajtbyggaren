...
Last verified state: `fa277a1` (2026-05-18, direktpushad `fix(builder): close B109 (UnicodeDecodeError safe-fallback in copy_starter)`). Builder-passet stängde B109 (extern reviewer-fynd i Cursor Bugbot-stil mot baseline `1c68035`): `_npm_install_inputs_changed` fångade bara `(OSError, json.JSONDecodeError)` men `load_json` läser med `encoding="utf-8"`, så en target-`package.json` med ogiltig UTF-8 raisade `UnicodeDecodeError` ut ur `copy_starter()` och kraschade builden i stället för det dokumenterade safe-fallback-beteendet. Fixen lägger till `UnicodeDecodeError` i except-tuple så alla tre läsningsfel ger samma `return True` (force reinstall); source-pkg-läsningen lämnas avsiktligt orörd (repo-kontrollerade starter-filer ska larma högt vid korruption). Två nya regressionstester i `tests/test_builder_hardening.py` (direkt enhetsligt + end-to-end via `copy_starter`). Aktuell buggräkning: 17 aktiva, 0 misplaced, 6 unknown, 78 stängda. Föregående mainline-commits: `0d3e9b8` (Steward-bump efter scope-cleanup), `7742d39` (Steward bug-scope cleanup, flyttade 15 misplaced 1B-fixar (`885431b`) till Stängda), `1c68035` (B108-fix), `860e553` (`feat(backoffice): add asset control plane`). Guards gröna: `python -m pytest tests/test_builder_hardening.py::test_npm_install_inputs_changed_falls_back_when_target_has_invalid_utf8 tests/test_builder_hardening.py::test_copy_starter_drops_node_modules_when_target_package_json_has_invalid_utf8 tests/test_builder_hardening.py::test_copy_starter_drops_node_modules_when_dependencies_change -v`, `python -m ruff check scripts/build_site.py tests/test_builder_hardening.py`, `python -m pytest tests/test_bug_scope_discipline.py tests/test_docs_freshness.py tests/test_no_legacy_terms.py -q`, `python scripts/list_open_bugs.py`, `python scripts/governance_validate.py`, `python scripts/rules_sync.py --check`, `python scripts/check_term_coverage.py --strict`. `backup-26-VIKTIG` (pre-B108), `backup-27` (post-B108, pre-cleanup) och `backup-28` (post-cleanup, pre-B109) finns pushade på origin; alla lokala branches utom `main` är raderade på operatörens uttryckliga begäran.
