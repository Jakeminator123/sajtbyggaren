...
Last verified state: `df24488` (2026-05-18, direktpushad `fix(scrape-runner): close B118 (add SIGKILL fallback to match build/prompt runners)`). Senaste produktpass var **andra rundan** av post-PR-#31 reviewer-triage: B117 (Medel — SVG-XSS via `/api/asset-preview`) och B118 (Låg — scrape-runner saknade SIGKILL-fallback) stängda; B119-B122 nyregistrerade som öppna (kontaktdata alfabetisk sortering, adress-till-stad-regex snäv, discovery-sanning splittrad i fyra lager som arkitekturskuld, UI thinking→building tidsheuristik). Föregående commits i första rundan av samma reviewer-pass: `fe9748e` (B114 early `Content-Length`-guard i `/api/upload-asset`), `cd03897` (B113 SSRF redirect-validation i `scripts/scrape_site.py` + 6 regressionstester i `tests/test_scrape_site_ssrf.py`), `d06e628` (pyright optional-narrowing cleanup i `_apply_discovery_overrides` — pure editor warning, inte runtime), `61da065` (`--discovery` + `--followup-site-id` rejection i `scripts/prompt_to_project_input.py`), `d63fab3` (BuildProgressCard `elapsedSec`-reset via `key={buildStage}` i `apps/viewser/components/viewer-panel.tsx`). PR #31 `feat(viewser): integrate christopher-ui discovery and asset workflow` är basen (merge `3f4543d`, integration `0510146`): hela `apps/viewser/components/discovery-wizard/**`, asset upload pipeline (`apps/viewser/lib/asset-store/**` + `/api/upload-asset` + `/api/asset-preview`), URL scrape (`scripts/scrape_site.py` + `/api/scrape-site` + `apps/viewser/lib/scrape-runner.ts`), SiteHeader/ConsoleDrawer, shadcn-primitives, `BUILD_TIMEOUT_MS` 3 min → 10 min, schema-fält `brand{logo, heroImage, primaryColorHex, accentColorHex, logoText, heroText}` + `gallery[]` + `$defs/assetRef` i `governance/schemas/project-input.schema.json`, naming-dictionary v15 → v16 (AssetRef, AssetStore, operator upload). Aktuell buggräkning: **25 aktiva, 0 misplaced, 6 unknown, 83 stängda** (B113/B114/B117/B118 stängda över två reviewer-rundor; B115/B116/B119/B120/B121/B122 nyregistrerade som öppna — se `docs/known-issues.md`). Guards gröna: `python -m ruff check .` (0 findings), `python scripts/governance_validate.py`, `python scripts/rules_sync.py --check`, `python scripts/check_term_coverage.py --strict`, `python -m pytest tests/` (675 passed, 3 skipped E2E), `npm run lint` + `npm run build` i `apps/viewser/` (11 routes inkl. nya `/api/asset-preview`, `/api/scrape-site`, `/api/upload-asset`). `backup-pre-christopher-ui-merge` finns pushad på origin som extra säkerhet före PR #31-mergen; taggen `archive/christopher-ui-2026-05-18` pekar på `4a16528` så hela christopher-ui-branchen kan återställas vid behov. `origin/christopher-ui` är raderad enligt operatörens policy om inga långa parallella branches. Inga öppna PRs.
...
`main` är vid `df24488` (`fix(scrape-runner): close B118 (add SIGKILL fallback to match build/prompt runners)`) ovanpå `6772a14` (B117 SVG-XSS via `/api/asset-preview`), `fe9748e` (B114 upload size guard), `cd03897` (B113 SSRF redirect-validation), `d06e628` (pyright cleanup), `61da065` (--discovery + --followup-site-id rejection), `d63fab3` (BuildProgressCard reset via key-prop) och PR #31:s merge-commit `3f4543d` som tog in hela christopher-ui-frontend-integrationen. Post-PR-#31-review har körts i **två rundor**: första rundan stängde B113 (SSRF) + B114 (upload-size) och öppnade B115 (binär-dubbletter) + B116 (`BUILD_TIMEOUT_MS` serialisering). Andra rundan stängde B117 (SVG-XSS via CSP sandbox + nosniff på `/api/asset-preview`) + B118 (scrape-runner SIGKILL-fallback matchande build/prompt-runner) och öppnade B119 (deterministic_fields kontaktdata via alfabetisk sortering — fel-men-plausibel-info), B120 (adress-till-stad-regex för snäv, bara svenska postnummerformat utan komma), B121 (medel arkitekturskuld — discovery-sanning passerar wizard → master-prompt → brief → overrides utan explicit konfliktlösning), B122 (UI thinking→building via `setTimeout(1500)` istället för backend-signal). Föregående baseline (B108-B112 + 1B-1E-fixar) kvarstår oförändrad i koden. Nästa steg för demo-kvalitet är fortfarande Re-Verifierings-Scout 5 med samma fyra demo-prompter, nu med möjlighet att testa discovery-wizardens AssetsStep + URL-scrape end-to-end. Sannolik nästa Builder-prio: B119 (kontaktdata-vägning) eftersom det påverkar genererad copy-kvalitet, sedan B121 (discovery-source-of-truth-helper) eftersom det är arkitekturlås mot framtida buggar.
