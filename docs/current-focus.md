...
Last verified state: `7742d39` (2026-05-18, direktpushad `docs: close 1B misplaced bug entries and bump scope counters` ovanpå produktcommit `1c68035` (`fix(builder): harden starter dependency baseline`)). Steward-städ flyttade alla 15 misplaced 1B-fixar (PR #28 / `885431b`) från "Öppna" till "Stängda" i `docs/known-issues.md`, bumpade summary-raden till 17 aktiva / 0 misplaced / 6 unknown / 77 stängda, och rättade 1B closure-noten så listan reflekterar de faktiska Fix-markörerna — B71/B72/B75/B83 är medvetet kvar som öppna trots PR #28-mergen (B71 markerad som unverified av re-Scout; B72/B75/B83 har `Fix: open` i sina poster). Guards gröna: `python scripts/list_open_bugs.py`, `python scripts/focus_check.py`, `python scripts/governance_validate.py`, `python scripts/rules_sync.py --check`, `python scripts/check_term_coverage.py --strict`, `python -m pytest tests/test_bug_scope_discipline.py tests/test_docs_freshness.py tests/test_no_legacy_terms.py -q`. Föregående produktcommit: `1c68035` (B108-fix). Tidigare: `860e553` (`feat(backoffice): add asset control plane`). `backup-26-VIKTIG` (pre-B108) och `backup-27` (post-B108, pre-cleanup) finns pushade på origin; alla lokala branches utom `main` är raderade på operatörens uttryckliga begäran.
