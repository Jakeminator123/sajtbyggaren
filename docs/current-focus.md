...
Last verified state: `aee67d7` (2026-05-21, **B144 Run Details warning-rendering landad; Steward-docs-syncen är inom bump tolerance**) — lokal `main` har `aee67d7` (`fix(viewser): close B144 - render site-plan warnings`) ovanpå `c2f0b0b`; `backup-40` finns på origin från pre-B144-läget. B144 är Scout-RO-godkänd och stängd: Run Details läser nu `sitePlan.pageCountWarning`, `sitePlan.intentGuardWarnings` och `sitePlan.pageIntentWarnings` från `site-plan.json` och renderar ett amber-block `data-testid="site-plan-warnings"` med svensk non-blocking-copy; source-locken `tests/test_viewser_files.py::test_run_details_panel_renders_site_plan_warnings` låser canonical källa + UI-yta. Bug-räkning efter B144: **29 aktiva, 0 misplaced, 5 unknown, 102 stängda**. **B143 är fortfarande öppen. PR #51 (`cursor/b143-intent-guard-slugs-5156`) ska inte mergas:** Scout RO-review 2026-05-21 gav verdict stop eftersom branchen bygger på pre-Intent-Guard-main, introducerar parallell `_intent_guard_warnings`/`_INTENT_GUARD_CONFLICTS`, får dead code efter merge, har add/add-konflikt i `tests/test_intent_guard.py`, duplicerar B143 i `docs/known-issues.md` och byter warning-shape bort från schema/B144-UI-kontraktet. Nästa B143-försök ska starta från aktuell `main`, bevara warning-shape `{categoryId, conflictingTerm, reason, businessTypeGuess}`, och utöka den befintliga helpern/tabellen med engelska sluggar eller bucket-normalisering. **PR #52** (`cursor/codegen-brief-data-ef0b`, B141) är separat cloud-grind-spår och får fortsätta om den rebases mot aktuell `main`, håller B141-scope och får egen Scout/CI-review före merge. **Direkt nästa orkestrator-fokus:** stäng/abandon PR #51 och starta ny B143-branch från current main, eller reviewa/mergea PR #52 först om den är ren och disjunkt; kör därefter mini-eval med `scripts/verify_run.py` innan Project DNA-beslut. Tidigare paragraf:
...
`main` är vid `aee67d7` lokalt (`fix(viewser): close B144 - render site-plan warnings`) med en efterföljande Steward-docs-sync inom bump tolerance. B144 är klar i kod + source-lock och ska pushas efter docs-guards. B143 är fortsatt öppen: PR #51 är stoppad av Scout eftersom den bygger på fel bas och skulle ge dead code/konflikter/inkompatibel warning-shape. B141-grindspåret finns som PR #52 och får fortsätta om det rebases mot aktuell `main` och inte rör B143/B144-ytor. Nästa produktsteg är ny B143-branch från current main eller review/merge av PR #52 om den är ren. Föregående stage snapshot:
...
Ingen pågående lokal produktimplementation efter B144. Aktivt orkestreringsläge: (1) push B144 + Steward-docs-sync, (2) stoppa/abandon PR #51 för B143, (3) starta om B143 slug-hardening från current `main` med befintlig warning-shape, (4) reviewa PR #52 för B141 separat om branchen rebases och håller scope. Viewser-overlay-E2E-mini-eval körs efter B143 eller efter B141 om den hinner landa först.
...
**Stoppa PR #51 som B143-lösning.** Den ska stängas/abandonas, inte mergas. Ny B143-agent ska starta från aktuell `main` och göra minsta korrekta fix i den befintliga Intent Guard-koden: bevara schema/B144-UI-shapen `{categoryId, conflictingTerm, reason, businessTypeGuess}`, utöka befintlig `_INTENT_GUARD_CONFLICTS` eller normalisera svenska/engelska slugs till buckets, behåll main:s nuvarande `tests/test_intent_guard.py` och lägg till slug-only-regressioner (`fitness` + `restaurant`, `salon/beauty` + `electrician`, `construction` + `hairdresser` utan `servicesMentioned`). Inga schemaändringar behövs om warning-shapen bevaras.
...
**PR #52/B141 får fortsätta parallellt** om den basas om mot current `main` efter B144-docs-pushen och håller sig till codegen/planning/test/docs för B141. Den får inte röra `scripts/build_site.py`, `tests/test_intent_guard.py`, B143-entries eller B144-UI-filer.
...
eller ersätt med ny branch från current `main`. PR #52
(`cursor/codegen-brief-data-ef0b`, B141) är inte blockerad här men kräver
egen review/CI och rebase mot B144-docs-HEAD före merge. Övriga äldre
PR-blockers är stängda/mergade: PR #38 mergades 2026-05-19
