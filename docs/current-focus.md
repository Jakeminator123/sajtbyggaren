...
Last verified state: `fe9748e` (2026-05-18, direktpushad `fix(upload): close B114 (early content-length guard in /api/upload-asset)`). Senaste produktpass tog hand om post-PR-#31 (christopher-ui-integrationen) review-fynd. Föregående commits i samma pass: `cd03897` (B113 SSRF redirect-validation i `scripts/scrape_site.py` + 6 regressionstester i `tests/test_scrape_site_ssrf.py`), `d06e628` (pyright optional-narrowing cleanup i `_apply_discovery_overrides` — pure editor warning, inte runtime), `61da065` (`--discovery` + `--followup-site-id` rejection i `scripts/prompt_to_project_input.py`), `d63fab3` (BuildProgressCard `elapsedSec`-reset via `key={buildStage}` i `apps/viewser/components/viewer-panel.tsx`). PR #31 `feat(viewser): integrate christopher-ui discovery and asset workflow` är basen (merge `3f4543d`, integration `0510146`): hela `apps/viewser/components/discovery-wizard/**`, asset upload pipeline (`apps/viewser/lib/asset-store/**` + `/api/upload-asset` + `/api/asset-preview`), URL scrape (`scripts/scrape_site.py` + `/api/scrape-site` + `apps/viewser/lib/scrape-runner.ts`), SiteHeader/ConsoleDrawer, shadcn-primitives, `BUILD_TIMEOUT_MS` 3 min → 10 min, schema-fält `brand{logo, heroImage, primaryColorHex, accentColorHex, logoText, heroText}` + `gallery[]` + `$defs/assetRef` i `governance/schemas/project-input.schema.json`, naming-dictionary v15 → v16 (AssetRef, AssetStore, operator upload). Aktuell buggräkning: **21 aktiva, 0 misplaced, 6 unknown, 81 stängda** (B113/B114 stängda i detta pass; B115/B116 nyregistrerade som öppna — binär-dubbletter `/public/` vs `apps/viewser/public/` och `BUILD_TIMEOUT_MS` 10 min UX-serialisering, se `docs/known-issues.md`). Guards gröna: `python -m ruff check .` (0 findings), `python scripts/governance_validate.py`, `python scripts/rules_sync.py --check`, `python scripts/check_term_coverage.py --strict`, `python -m pytest tests/` (675 passed, 3 skipped E2E), `npm run lint` + `npm run build` i `apps/viewser/` (11 routes inkl. nya `/api/asset-preview`, `/api/scrape-site`, `/api/upload-asset`). `backup-pre-christopher-ui-merge` finns pushad på origin som extra säkerhet före PR #31-mergen; taggen `archive/christopher-ui-2026-05-18` pekar på `4a16528` så hela christopher-ui-branchen kan återställas vid behov. `origin/christopher-ui` är raderad enligt operatörens policy om inga långa parallella branches. Inga öppna PRs.
...
`main` är vid `fe9748e` (`fix(upload): close B114 (early content-length guard in /api/upload-asset)`) ovanpå `cd03897` (B113 SSRF redirect-validation), `d06e628` (pyright cleanup), `61da065` (--discovery + --followup-site-id rejection), `d63fab3` (BuildProgressCard reset via key-prop) och PR #31:s merge-commit `3f4543d` som tog in hela christopher-ui-frontend-integrationen (discovery wizard, asset upload pipeline, scrape-route, schema-fält för brand/gallery, naming-dictionary v16). Post-PR-#31-review identifierade två säkerhetsrelaterade fynd som båda stängdes i samma pass: B113 (Hög) — SSRF-glipa där `fetch_html` följde redirects utan ny `validate_ssrf`-koll, och B114 (Låg) — `/api/upload-asset` buffrade hela payloaden via `request.formData()` innan storlekscheck mot `file.size > MAX_FILE_BYTES`. Två nya öppna fynd registrerade: B115 (Låg) — binär-dubbletter (`SM_hero.mp4` + `LOGO_SM2.0.png` finns både i `/public/` och `apps/viewser/public/`, ~3.4 MB duplicerat) och B116 (Låg) — `BUILD_TIMEOUT_MS` 10 min i `apps/viewser/lib/build-runner.ts` är globalt serialiserad via `inFlight`-mutex så en hängd build blockerar alla nya prompter i upp till 10 minuter. Föregående baseline (B108-B112 + 1B-1E-fixar) kvarstår oförändrad i koden. Nästa steg för demo-kvalitet är fortfarande Re-Verifierings-Scout 5 med samma fyra demo-prompter, nu med möjlighet att testa discovery-wizardens AssetsStep + URL-scrape end-to-end.
