...
Last verified state: `1b91ca6` (2026-05-26 kväll UTC, post-Gap-5+Gap-4-close — `directives.notesForPlanner` + `directives.requestedCapabilities` når nu planningModel deterministiskt; backend-handoff-status 7 stängda / 3 delvis / 1 öppen).
...
- `b89a3d2` feat(discovery): persist directives.notesForPlanner into Site Brief (Gap 5). Schema-fält + resolver-trim/cap + `_apply_operator_directive_note()`-helper i `scripts/build_site.py` som prepend:ar operator-noten med prefix `"Operator: "` framför briefens egen `notesForPlanner` i både mock- och real-path. 3 resolver-tester + 2 builder-smoke-tester.
- `1b91ca6` feat(discovery): merge directives.requestedCapabilities into resolver (Gap 4). Schema-fält + sanitering (string, trim, dedup, max 32) i `_apply_directives_fields()` + ny första loop i `_resolve_capabilities()` som tar `directive_caps` FÖRE `wizard_caps` med source-label `"wizard"`. 4 resolver-tester (merge + dedupe + unknown-warning + 32-item-cap).
...
Inga öppna PRs på `jakob-be` eller `main`. PR #116 (`8c057b1`), PR #117 (`2319ef9`), PR #118 (`1740472`), PR #119 (`31d8d74`), PR #120 (`15aea46`) och PR #121 (cloud-grind C4 audit, stängd via lokal merge `0f3bd67`) är alla mergade/landade. `jakob-be` är resetad till `origin/main` 2026-05-26 PM (commit `1004122`) plus följande direkta commits ovanpå: `cc1a5aa` (vercel.json), `0ed5348` (backend-handoff cleanup audit), `3fc187e` (focus bump efter cleanup), `4cd367c` (naprapat plan-fallback fix), `b414c6b` (naprapat pick_scaffold + eval routes fix), `ee1751f` (focus bump + Lane 2/Lane 4 stale-correction), `d3a2ad6` (reviewer-flagged drift correction), `9dbd10a` (drop fabricated wizard-product type ref), `0f3bd67` (cloud-grind C4 audit landed via local merge with conflict resolution), `6aeec35` (evening handoff till nästa orchestrator), `fdb1fef` + `ff6154e` (handoff-doc text-fixar mot term-coverage), `b89a3d2` (Gap 5 close: `directives.notesForPlanner` persist + brief-merge), `1b91ca6` (Gap 4 close: `directives.requestedCapabilities` resolver-merge). `jakob-be` är 14 commits framför `origin/main` — nästa sync-PR till main när nästa större spår är inne (B147-fixen eller någon av backend-Gap 6/7/9/10-fixarna nedan).

Pending operatörsklick: inga (Vercel production-branch är flippad till `main` per operatörsbekräftelse 2026-05-26 kväll; `docs/operations/vercel-production-branch-todo.md` raderad).
...
1. **B147 vägval a/b/c** (operatörsbeslut, sen kod). Vercel preview wizard 403 via `assertLocalhost` på `*.vercel.app`. Tre alternativ i `docs/known-issues.md`: (a) `VIEWSER_ALLOW_NON_LOCALHOST=true` på Vercel-projektets Preview- + Production-env (snabbast, men bekräftar `no auth, no rate limit, no public deploy`-modellen på publik URL), (b) host-whitelist via ny `VIEWSER_ALLOWED_HOSTS`-env (mer kontrollerat), (c) ADR-beslut om Viewser-på-Vercel auth-strategi (långsiktig).
2. **Backend-Gap fixar baserade på C4-audit** (cloud-grind levererade noggrann audit 2026-05-26 i PR #121, `0f3bd67`). Status efter Gap-5+Gap-4-close 2026-05-26 kväll (`b89a3d2` + `1b91ca6`): 7 stängda (1, 2, 3, 4, 5, 8, 11), 3 delvis (6, 7, 9), 1 öppen (10). Konkreta nästa fixar i prioritetsordning:
   - Gap 6 + 7 paired sprint (~3-4h, M): build-pipeline-konvertering. Gap 6 = multi-size `public/favicon.ico` från `media.favicon` (Next metadata `icons` redan rendrad i `packages/generation/build/renderers.py:313-331`). Gap 7 = center-crop till `public/og-image.png` 1200×630 från `media.ogImage` (Next OG-metadata redan i `renderers.py:336-367`). Båda kräver pillow eller sharp i build.
...
3. **Sync-PR `jakob-be → main`** när nästa större spår eller flera quick-wins är inne. `jakob-be` är 5 commits framför `origin/main` efter denna sessions Gap-5+Gap-4-close (3 doc-commits från tidigare evening-session + 2 nya feat-commits + 1 doc-bump-commit som denna handoff producerar).
