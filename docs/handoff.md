...
Se `docs/current-focus.md` → **"Next action"**. Kort version: B121 discovery-integration är stängd (PR #34–#37, Fix-SHA `e3fa67b`). Nästa är **Re-Verifierings-Scout** — fyra produktbaseline-prompter, visuell/kvalitativ bedömning via nya overlayflödet, jämfört mot Scout-4-baseline 6.59/10. Vid ≥7/10 och inget case <6.5 → Project DNA-sprint.

Kända lågprio-rester (oförändrat): B101/B102 (commerce-CTA route/text), B97 (`/kontakt`-copy), B98 (bredare e-handelsserviceområde-yta; B104 stängde bara country-only-läckan). B119/B120 (kontakt/adress) prioriteras om Scout visar fel kontaktdata. B125 (browser-preview-fallback) är produktblocker före extern kundyta.
