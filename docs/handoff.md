...
Se `docs/current-focus.md` → **"Next action"**. Kort version: PR #32-cherrypick-serien + `0fe353f` (B126/B127) har gjort backoffice-kontrollplanen hel; starter dependency hardening (B108) och demo-baseline-fix 1E (B105/B106/B107) ligger kvar som tidigare baseline. Nästa feature-spår är **B121 — Discovery Resolver / canonical discovery source-of-truth**: bygg en delad resolver-helper med deterministisk "wizard wins, brief, LLM-default"-ordning per fält så att discovery-sanningen inte längre splittras i fyra lager (wizard payload → tempfil → briefModel → `_apply_discovery_overrides`) utan explicit konfliktlösning. Bekräfta placering mot repo-boundaries innan kod — om resolvern hör hemma i `packages/generation/` ska sprinten ta en mini-ADR ovanpå ADR 0017/0019 så den inte krockar med B13a-arkitektur-flytten. Efterföljande sprint (egen, ej blocker): **backoffice discovery control / mapping review** — vy som visar `wizardAnswers` ⇄ `brief` ⇄ `appliedOverrides` ⇄ `finalProjectInput` per run så operatören kan inspektera vilken källa som vann per fält.

Re-Verifierings-Scout 5 + Project DNA flyttas ned i kön — de är fortfarande värdefulla men ger inte arkitekturlås på samma sätt som B121. Kända lågprio-rester (oförändrat): B101/B102 (commerce-CTA route/text), B97 (`/kontakt`-copy), B98 (bredare e-handelsserviceområde-yta; B104 stängde bara country-only-läckan).
