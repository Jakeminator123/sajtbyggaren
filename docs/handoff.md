...
- 5 automatiska checks: `governance_validate.py`, `rules_sync.py`, `check_term_coverage.py --strict`, `pytest`, `ruff check .`. GitHub Actions kör alla på push/PR. `tests/test_docs_freshness.py` är en sjätte mjuk guard som fångar drift mellan narrativ docs (`AGENTS.md`, `dossiers/README.md`) och verkligheten — tillagd i post-audit-2 (B25/B26), utökad i audit-3 (B27) och audit-4 (B28).
...
- **Post-audit-2 cleanup (efter `e8143cf`):** B23/B24/B25/B26 stängda. `build_plan_artefakts` har nu source-regex-skydd för "validate efter merge"-ordningen (Bug C end-to-end), `merge_operator_selected_with_helper(operator=list)`-pathen är direkt testad (Bug A coverage gap), `AGENTS.md` ruff-claim auto-verifieras mot verkligheten via `tests/test_docs_freshness.py`, och `dossiers/README.md` implementations-status auto-verifieras mot disk-walk. Hela docs↔kod-driftytan är nu testbar.
- **Audit-3 (`eaf2d91`)** stängde B27: `_id_appears_as_token()`-helpern i `tests/test_docs_freshness.py` förhindrar att överlappande Dossier-IDs (typ `game` inuti `interactive-game-loop`) ger falsk-positiv. **Audit-4 (`f3dc5c2`)** stängde B28: ruff-output-regex använder nu explicit `errors?` istället för att lita på substring-prefix-tillfällighet.
- **Sprint 3A (ADR 0015):** ny fas-3-yta under `packages/generation/`. `codegen/` emittar deterministisk CodegenResult-manifest (LLM kommer i 3B), `quality_gate/` kör typecheck + route-scan + build-status + policy-compliance och skriver riktiga `quality-result.json` (inte längre skeleton `status=not-run`), `repair/` tar quality-resultatet och returnerar strukturerat `RepairResult` (`not-needed` när allt är ok, `no-fix-applied` med `remainingErrors[]` när något failar). `scripts/build_site.py` är tunn wiring; produktlogik bor i paketen. `engine-run.v1.json` bumped till v3, `repo-boundaries.v1.json` till v8 (mappnamn använder underscore för Python-import-kompatibilitet). Skeleton-funktionerna `write_repair_result_skeleton`/`write_quality_result_skeleton` är borta och regression-skyddade av `tests/test_build_site_size.py`.
...
- Fas 3 LLM-bitar: real `codegenModel`-anrop (Sprint 3B), full Repair Pipeline med mekanisk Fix Registry + LLM-fix (Sprint 3B/5), Quality Gate-scoring mot Page Quality Traits (Sprint 3C), och Preview Runtime (Sprint 4-5) är fortfarande inte implementerade. Sprint 3A landade deterministisk `codegenModel v1`-manifest, riktiga Quality Gate-checks (typecheck + route-scan + build-status + policy-compliance) och no-fix-applied Repair Pipeline under `packages/generation/{codegen,quality_gate,repair}/`.
