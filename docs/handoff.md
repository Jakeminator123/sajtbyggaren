...
**Datum:** 2026-05-18 (post-B109 reviewer-hotfix ovanpå Steward bug-scope-cleanup, starter dependency hardening, Backoffice control-plane och demo-baseline-fix 1E. `fa277a1` `fix(builder): close B109 (UnicodeDecodeError safe-fallback in copy_starter)` stängde extern reviewer-fynd: `_npm_install_inputs_changed` fångar nu `UnicodeDecodeError` tillsammans med `(OSError, json.JSONDecodeError)` så en target-`package.json` med ogiltig UTF-8 triggar safe-reinstall i stället för build-crash. Två regressionstester låser fixen. `1c68035` `fix(builder): harden starter dependency baseline` stängde tidigare B108. Aktuellt bug-scope: 17 aktiva, 0 misplaced, 6 unknown, 78 stängda. Nästa konkreta uppgift är fortfarande **Re-Verifierings-Scout 5** med samma fyra demo-prompter.)
**Aktuell repo-HEAD på `main`:** `fa277a1` (`fix(builder): close B109 (UnicodeDecodeError safe-fallback in copy_starter)`) + en docs-bump-commit ovanpå; kör `git log --oneline -1` eller `python scripts/focus_check.py` för faktisk HEAD-SHA. Föregående relevanta commits: `0d3e9b8` (Steward focus-bump efter scope-cleanup), `7742d39` (Steward bug-scope cleanup), `9f13ccc` (focus-bump efter B108), `1c68035` (B108-fix), `9cb32cf` (Backoffice docs bump), `860e553` (Backoffice control-plane), `bc43eb8` (1E fix), `01c0cfb` (variant candidate generator), `9cc3067` (1D fix), `48d6e24` (`Lite allt möjligt`), `6eaf222` (1C Steward-bump), `b5ee710` (1C fix).
**Aktiv branch:** `main`. `backup-26-VIKTIG` (pre-B108), `backup-27` (post-B108, pre-cleanup) och `backup-28` (post-cleanup, pre-B109) är pushade till origin, men alla lokala branches utom `main` är raderade på operatörens uttryckliga begäran. PR #29 och PR #30 är mergade sedan tidigare; PR-brancherna `cursor/bug-scope-disciplin` och `cursor/backoffice-rensning-styrning-7c51` är raderade både lokalt och remote. Inga öppna PRs.
