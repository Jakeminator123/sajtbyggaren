...
3. `Quality Gate` finns på **exakt en** plats: `packages/generation/quality_gate/`. **En** gate; inga parallella tier-axlar (`naming-dictionary.v1.json:qualityGate.definition` + `preview-runtime-policy.v1.json:principles`).
4. `Preview Runtime` är en abstraktion. Produktkoden talar bara om `Preview Runtime`, aldrig om implementationsnamnen `VM`, `webcontainer`, `preview-host` eller oregistrerade `sandbox`-synonymer. Registrerade alias (`Vercel Sandbox`, `sandbox` under Preview Runtime) finns i [`naming-dictionary.v1.json`](../policies/naming-dictionary.v1.json). **Faktisk default idag:** `local`/`local-next` (dev + fallback). **Primärt förstahandsval (ADR 0033):** `VercelSandboxRuntime` via explicit `VIEWSER_PREVIEW_MODE`. `StackBlitzRuntime` är pausad, inte default.
