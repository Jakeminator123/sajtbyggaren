...
1. Skriv meddelandet till `$env:TEMP\sb-commit-msg.txt` (utanför repo).
2. `git commit -F $env:TEMP\sb-commit-msg.txt`.
3. `Remove-Item $env:TEMP\sb-commit-msg.txt` (städ; men även om du
   glömmer ligger filen utanför repo).
...
(`.tmp-*`, `.tmp.*`, `.commit-msg.tmp`), men prevention via `$env:TEMP`
är säkrare än att lita på ignore-mönstret.
