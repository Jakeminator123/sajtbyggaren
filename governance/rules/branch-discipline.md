---
description: Jakob arbetar på `jakob-be`, Christopher på `christopher`, `main` är canonical. PR mot `main` när en ny officiell version ska in.
alwaysApply: true
---
...
- **Christopher** jobbar default på arbets-branchen `christopher`
  (UI/frontend i `apps/viewser/`, presentationslagret för genererade
  sajter).
- `christopher-ui` är **fryst legacy** och får inte röras utan operatörens
  uttryckliga OK: den bär gammal historik plus parkerad auth/billing bakom
  `NEXT_PUBLIC_AUTH_ENABLED` (default AV) som medvetet tas in långt senare.
  Den auktoritativa källan för Christopher-lanens branch-val är
  [`christopher-active-branch.md`](christopher-active-branch.md).
- **`main`** är projektets canonical / sanningsbranch. Pushas aldrig med
  `--force`. Direkt-pushes till `main` är undantag, inte standard.
- PR från `jakob-be` eller `christopher` mot `main` öppnas när en ny
...
1. Verifiera att aktuell branch är `jakob-be` (Jakob) eller
   `christopher` (Christopher) — `git branch --show-current`.
...
6. `git push origin jakob-be` (eller `christopher`).
...
3. `git push --force-with-lease origin <branch>` — uppdaterar
   `origin/jakob-be` (eller `origin/christopher`) till nya
   `origin/main`-spetsen.
...
inte backup-branches på eget initiativ — `jakob-be` och `christopher`
...
- Skapar permanenta branches utöver `jakob-be` / `christopher` /
  `backup-N`.
...
1. Stoppa direkt — ingen automatisk `--force` på `main`. På
   `jakob-be`/`christopher` är `--force-with-lease` OK enligt
...
- Jobbar på sin egen arbets-branch (`jakob-be` respektive
  `christopher`) och rör inte motpartens branch.
...
- **Operatörsgrant (Jakob, 2026-06-08) — lane-unblock, INTE carte blanche.**
  Operatören har gett jakob-be rätt att gå in i Christophers UI-lane
  (`apps/viewser/**`) i två fall:
  - **Stående buggfix-tillstånd:** jakob-be FÅR fixa buggar det upptäcker i
    Christophers lane utan per-bugg-förhandsgodkännande — committat på
    `jakob-be`, taggat `[scope-leak] Approved by operator`, och rapporterat i
    `docs/agent-inbox.jsonl`. "Om buggar finns, fixa dem i jakob-be om du ser
    dem" (operatörsdirektiv 2026-06-08). Gäller buggfixar, inte nya features.
  - **Riktat unblock per ändring (större/icke-bugg):** för en *specifik*
    feature-/UX-ändring krävs fortfarande att (1) den hör till kärnloopen
    `prompt -> sajt -> preview -> följdprompt`, (2) operatören explicit godkänt
    just den ändringen, (3) diffen är liten och rapporteras, (4) Christopher
    informeras i inbox/handoff.
  I båda fallen består lane-ägandet (Christopher är default-ägare av UI/UX och
  informeras), och commit-body taggas `[scope-leak] Approved by operator:
  <motivering>`. Det är inte stående full access för godtyckliga ändringar.
...
- Sin egen arbets-branch (`jakob-be` eller `christopher` beroende på
...
Mellan `jakob-be` och `christopher` finns ingen push-race eftersom
...
1. `git branch --show-current` — verifiera rätt branch (`jakob-be`,
   `christopher` eller `main` för Steward-undantag).
