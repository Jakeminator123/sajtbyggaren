...
`Missing file specification after redirection operator`. För att
undvika att skapa skräpfiler i repo-roten:

1. Skriv meddelandet till `$env:LOCALAPPDATA\Temp\sb-commit-msg.txt`
   (utanför repo, alltid user-temp). Använd helst en unik filnamns-
   suffix per commit, t.ex. tidsstämpel: `sb-commit-msg-<yyyyMMddHHmmss>.txt`.
2. `git commit -F $env:LOCALAPPDATA\Temp\sb-commit-msg.txt`.
3. `Remove-Item $env:LOCALAPPDATA\Temp\sb-commit-msg.txt` (städ; men även
   om du glömmer ligger filen utanför repo).
...
> with taxonomy`. Erratan landade i `848b...`-serien.
...
(`.tmp-*`, `.tmp.*`, `.commit-msg.tmp`), men prevention via
`$env:LOCALAPPDATA\Temp` är säkrare än att lita på ignore-mönstret.
