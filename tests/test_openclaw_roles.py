...
def test_component_builder_is_supported_with_generative_recipe():
    ...
    default with the whitelisted deterministic recipes as the visible exceptions
    (the honest no-op stays for every non-recipe component_add)."""
    contract = contract_for_role("component_builder")
    assert contract.status == "supported"
    # Mount-only default + visible generative recipes (mirrors section_builder).
    assert contract.mountOnly is True
    assert contract.visibleTypes == ("image-placeholder-grid", "cta-contact-block")
